	...
	public class CommentServiceTests
	{
		...
		public async Task SetUp()
		{
			...
			_comments = new List<Comment>()
			{
				...
				{
					Id = 3,
					Content = "This is comment to different plant",
					ApplicationUserId = "8d16841e-7c17-44c5-b30d-5f10b43122c8",
					CreatedOn = DateTime.UtcNow,
					PlantId = 4
				},
		   ...
		   .UseInMemoryDatabase(databaseName: "PlantInMemoryDb" + Guid.NewGuid().ToString())
			...
			_service = new CommentService(_repository, _mapper);
		}
		...
		[Test]
		public async Task Test_GetCommentsAsync_Returns_Correct_Result()
		{
			const int id = 2;

			var comments1 = await _repository
				.AllReadOnly<Comment>()
			...
			var result = await _service.GetCommentsAsync(id);

			Assert.That(comments.Count, Is.EqualTo(result.ExistingComments.Count));
			Assert.That(_comments[0].Content, Is.EqualTo(result.ExistingComments[0].Content));
			Assert.That(_comments[1].Content, Is.EqualTo(result.ExistingComments[1].Content));
			Assert.That(Assert.ThrowsAsync<NotFoundException>(() => _service.GetCommentsAsync(6)), Is.Not.Null);
			Assert.That(result.ExistingComments, Is.TypeOf<List<CommentViewModel>>());
		}

		[Test]
		public async Task Test_AddCommentsAsync_Adds_Comment()
		{
			...
			await _service.AddCommentsAsync(model, id, plantId);

			var comments = await _repository
				.AllReadOnly<Comment>()
				.ToListAsync();

			Assert.That(comments.Count, Is.EqualTo(_comments.Count + 1));
			Assert.That(comments[3].Content, Is.EqualTo("This is a comment"));
			Assert.That(comments[3].ApplicationUserId, Is.EqualTo(id));
			Assert.That(comments[3].PlantId, Is.EqualTo(plantId));
			Assert.That(comments[3].CreatedOn, Is.EqualTo(DateTime.UtcNow).Within(1).Seconds);
		}

		[Test]
		public async Task Test_AddCommentsAsync_Throws_Exception_When_Invalid_Id()
		{
			var model = new CommentModel
			{
				Content = "This is invalid comment",
			};

			var id = "1";
			const int plantId = 2;

			Assert.That(Assert.ThrowsAsync<ArgumentException>(async () => await _service.AddCommentsAsync(model, id, plantId)), Is.Not.Null);
		}
	}
}
